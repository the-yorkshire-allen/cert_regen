...
plan cert_regen::clear_agent_cert (
  TargetSpec $targets,
  TargetSpec $primary,
) {
  $clear_certificates_result = run_task('cert_regen::clear_node_cert', $targets, '_catch_errors' => true)

  $agent_names = join($clear_certificates_result.ok_set.names,' ')

  $remove_result = run_task('cert_regen::remove_certificate', $primary, '_catch_errors' => true, 'agent_certnames' => $agent_names, 'purge_data' => false)
